...
namespace Presentation
{
    class BackendController
    {
        ...
        internal void Register(string username, string password, string nickname,string regemail)
        {
            Response res = Service.Register(username, password, nickname,regemail);
            if (res.ErrorOccured)
            {
                throw new Exception(res.ErrorMessage);
            }
        ...
        */
        ...
        internal Response<Task> GetTask(string email, int colordianl, int taskid )
        {
            Response<Task> res = Service.GetTaskById(email, colordianl,taskid);
            if (res.ErrorOccured)
            {
                throw new Exception(res.ErrorMessage);
            }
        ...
        internal Response AssignTask(string email, int col,int taskid, string emailAssignee)
        {
            Response res = Service.AssignTask(email, col, taskid,emailAssignee);
            if (res.ErrorOccured)
            {
                throw new Exception(res.ErrorMessage);
            }
        ...
        internal Response ChangeTitle(string email, int col, string newname)
        {
            Response res = Service.ChangeColumnName(email, col, newname);
            if (res.ErrorOccured)
            {
                throw new Exception(res.ErrorMessage);
            }
        ...
        internal Response DeleteTask(string email, int col, int taskid)
        {
            Response res = Service.DeleteTask(email, col, taskid);
            if (res.ErrorOccured)
            {
                throw new Exception(res.ErrorMessage);
            }
